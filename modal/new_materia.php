...
if(isset($_POST['add'])) {
            ...
            if(empty($errors)) {
                ...
                $query  = "insert into materias (descripcion) values (";
                $query .=" '{$descripcion}'";
                $query .=")";

                $result   = $db->query($query);
                if($result && $db->affected_rows() === 1){
                    $session->msg('s',"Materia ingresado exitosamente. ");
                } else {
        ...
        <div class="panel-heading clearfix">
            <div class="pull-left">
                <button type="button" class="btn btn-primary" data-toggle="modal" data-target=".bs-example-modal-lg-add">
                    <i class="fa fa-plus-circle"></i> Agregar Materia  <span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>
                </button>
            </div>
        </div>
