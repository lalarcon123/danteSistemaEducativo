...
function find_all_estudiantes($table) {
   global $db;
   if(tableExists($table))
   {
     return find_by_sql("SELECT id, concat(name,' ',last_name) AS nombres FROM ".$db->escape($table)." WHERE status = 1 and user_level =3 order by 2");
   }
}
...
function find_all_cursos($table, $id_user) {
   global $db;
   if(tableExists($table))
   {
     return find_by_sql("select co.id , CONCAT(c.descripcion, ' Fecha Inicio: ', co.fecha_inicio , ' Fecha Fin: ', co.fecha_fin, ' Estado: ', (case co.estado WHEN 1 THEN 'ACTIVO' WHEN 0 THEN 'INACTIVO' ELSE 'FINALIZADO' END)) as descripcion from ".$db->escape($table)." co INNER JOIN curso c on (c.id = co.id_curso) where (co.id_docente = '{$db->escape($id_user)}' or '{$db->escape($id_user)}' = 1) and co.estado in (1,2)");
   }
}
